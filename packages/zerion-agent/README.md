...
# Bundie agents on the Zerion vault (see section below)
zerion-bundie agent create --name <role>
zerion-bundie agent list
zerion-bundie agent sign --name <role> --tx <base64>
zerion-bundie chaos-sim-migrate [--keys-dir <path>]
```

## Bundie agents on the Zerion vault

Bundie agents (chaos-sim creators/traders, future autonomous strategies) ARE
Zerion-managed wallets. Every agent is provisioned via OWS
([@open-wallet-standard/core](https://www.npmjs.com/package/@open-wallet-standard/core))
under the hood — the private key never leaves the vault directory and
agent transactions are signed inside the OWS process boundary.

### Vault location

Default: `~/.ows/wallets/` (one encrypted file per wallet, OWS-managed).

Override with `BUNDIE_AGENT_VAULT_PATH=/path/to/vault` — this is what the
unit tests use to stay hermetic. The OWS Rust core supports a
`vaultPathOpt` argument on every public function.

### Naming convention

Every Bundie agent is stored as `bundie/<role>` in the vault (e.g.
`bundie/creator-0`, `bundie/trader-3`). The prefix lets `agent list`
filter the vault without depending on a metadata-tag feature OWS does not
currently expose.

### Passphrase

Bundie agents are unattended (chaos-sim runs in CI). The single
passphrase used to encrypt + sign with every agent comes from
`BUNDIE_AGENT_PASSPHRASE` (default empty, which OWS treats as "no
passphrase"). For a hardened deployment, set this in your CI secret store
...
| -------------------------------------------------- | ------------------------------------------------------------------------- |
| `agent create --name <role>`                       | Provision a vault wallet. Idempotent — returns the existing one if any.   |
| `agent list`                                       | List every Bundie agent in the vault. Filters by `bundie/` name prefix.   |
| `agent sign --name <role> --tx <base64>`           | Sign a base64-encoded Solana tx with the named agent's vault key.         |
| `chaos-sim-migrate [--keys-dir <path>]`            | Import existing `keys/<role>.json` files into the vault. Idempotent.      |

`agent sign` returns `{ ok: true, role, signedTx }` — the `signedTx` is
base64-encoded and ready for `connection.sendRawTransaction`. The agent's
positional signature is spliced back into the original tx, so any
embedded ephemeral signers (e.g. mint keypair from a `create-strategy`
prepare envelope) are preserved.

### chaos-sim migration

The chaos-sim wallet pool used to live in
`packages/programs/scripts/chaos-sim/keys/<role>.json` files generated by
`Keypair.generate()`. With the Zerion-vault migration, those files become
a fallback path while the canonical home is the OWS vault.

3-step migration script:

```bash
pnpm --filter @bundie/zerion-agent build
node packages/zerion-agent/src/cli.js chaos-sim-migrate
pnpm --filter @bundie/programs chaos:doctor   # should now show vault-managed wallets
```

`chaos-sim-migrate` is idempotent — re-running reports each role as
`already-in-vault`. It DOES NOT delete the legacy `keys/<role>.json`
files; the operator should delete them manually after verifying the
migration with `chaos:doctor`. The chaos-sim's `setupPool()` resolves
roles vault-first, file-fallback-second, and DENYs (throws — never
silently calls `Keypair.generate()`) when neither is available.

### How chaos-sim uses the vault at runtime

The chaos-sim's `cli-runner.ts` switches on `wallet.signWith`:

- `signWith === 'file'` → bundie-sol gets `--keypair <path> --execute`
  (legacy fallback while the migration is in progress).
- `signWith === 'zerion-vault'` → bundie-sol is invoked WITHOUT
  `--execute` (prepare-only). The harness pipes the resulting `PreparedTx`
  to `zerion-bundie agent sign`, then broadcasts the signed bytes via
  `connection.sendRawTransaction`. **bundie-sol stays unchanged** — the
  Zerion-mediated signing is explicit and lives outside its `--execute`
  path.
...
```bash
pnpm --filter @bundie/zerion-agent test:bundie         # 47 Bundie tests
pnpm --filter @bundie/zerion-agent test:bundie:vault   # 11 agent-vault tests
pnpm --filter @bundie/zerion-agent test                # all (incl. upstream)
```
...
src/bundie/agent-vault.js               # OWS-backed Bundie-agent primitives
...
tests/unit/bundie-agent-vault.test.mjs  # Bundie-agent vault + CLI tests
