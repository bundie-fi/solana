...
pub const RAYDIUM_CPMM_DEVNET_PROGRAM_ID: Address =
    Address::from_str_const("DRaycpLY18LhpbydsBWbVJtxpNv9oXPgjRSfpF2bWpYb");

const SWAP_BASE_INPUT_DISCRIMINATOR: [u8; 8] = [143, 190, 90, 218, 196, 30, 51, 222];

pub struct RaydiumCpmmDevnet;

impl RaydiumCpmmDevnetSwapAccounts<'_> {
    pub const NUM_ACCOUNTS: usize = 14;
}

pub struct RaydiumCpmmDevnetSwapAccounts<'info> {
...
impl<'info> TryFrom<&'info [AccountView]> for RaydiumCpmmDevnetSwapAccounts<'info> {
    ...
    fn try_from(accounts: &'info [AccountView]) -> Result<Self, Self::Error> {
        ...
        Ok(RaydiumCpmmDevnetSwapAccounts {
...
impl<'info> Swap<'info> for RaydiumCpmmDevnet {
    type Accounts = RaydiumCpmmDevnetSwapAccounts<'info>;
    type Data = ();
    ...
    fn swap_signed(
        ...
        let instruction = InstructionView {
            program_id: &RAYDIUM_CPMM_DEVNET_PROGRAM_ID,
            accounts: &accounts,
